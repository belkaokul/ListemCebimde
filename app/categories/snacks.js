...
import snacksPic from "../../assets/snacks.png"

const snacks = () => {

  const [newItem, setNewItem] = useState('');
  const loadFromStorage = useSnacksStore((state) => state.loadFromStorage);
  const addItem = useSnacksStore((state) => state.addItem);
  const data = useSnacksStore((state) => state.data);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Atıştırmalıklar</Text>
      <View style={{ borderBottomColor: 'black', borderWidth: 1, width: '100%' }} />

      <ScrollView contentContainerStyle={styles.cards}>
        {data.map((item) => (
          <MyCard2 key={item.id} header={item.header}  productName={item.header} src={snacksPic} />
        ))}
      </ScrollView>
